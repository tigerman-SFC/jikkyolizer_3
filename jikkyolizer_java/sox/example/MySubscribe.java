...
public class MySubscribe implements SoxEventListener {
	...
	File file = new File("/home/jikkyolizer/jikkyolizer_java/log/MySubscribe.log");
	...
	public static String dummy_escape(String not_escaped) {
		...
		Matcher matched;
		for(int i=0; i<escape_candidate.length; i++){
	...
	public MySubscribe() throws Exception {
		...
		//Avoid crazy length of meda id and name
		if (deviceInfo == null){ continue; }
		if (deviceInfo.getId() == null || deviceInfo.getName() == null ){ continue; }
		if (deviceInfo.getId().length() > 256 || deviceInfo.getName().length() > 256){ continue; }
		//System.out.println("[Device Meta Info] ID:" + deviceInfo.getId()
		outBuffer.write("Created Number is " + upper_limit + "\n");
		...
		for (Transducer t : transducerList) {
			...
			//String for_register_transducer = "'group_id=" + deviceInfo.getName() + ",item_id=" + t.getName() + "'";
			String for_register_transducer = "'group_id=" + deviceInfo.getName() + ",item_id=" + t.getName() + ",group_name=" + deviceInfo.getName() + "'";
			...
			String register_transducer[] = {"python3.6", "/home/jikkyolizer/jikkyolizer_input/jikkyolizer_device.py", for_register_transducer, "1>>/var/log/jikkyolizer/java_device.log", "2>>/var/log/jikkyolizer/java_device.error"};
		...
		if (data == null){ 
			outBuffer.write("--Above is null node--");
			continue;
		}
	...
	public void handlePublishedSoxEvent(SoxEvent e) {
		...
		for (TransducerValue value : values) {
			...
			p = new ProcessBuilder("python3.6", "/home/jikkyolizer/jikkyolizer_input/jikkyolizer_input.py", to_python_arg, "1>>/home/jikkyolizer/jikkyolizer_input/log/processing_input.log").start();
			ret = p.waitFor();



		outBuffer.write("python3.6 /home/jikkyolizer/jikkyolizer_input/jikkyolizer_input.py " + to_python_arg + " 1>>/home/jikkyolizer/jikkyolizer_input/log/processing_input.log");
		}
		outBuffer.flush();
