...
class JikkyolizerVocalize():
	def __init__(self, item_id, group_id, raw_value, row_timestamp, reason, vocalize_prior, is_change, rel_flag, relations):
		...
		vocalize_sentence += group_id + 'の' + item_id + 'の値が'
		...
		cmd = 'curl "https://api.voicetext.jp/v1/tts" -u "mpbw1hnrp32pdde7:" -d "text= \'' + raw_value + '\'" -d "speaker=hikari" -d "speed=95" -o /var/www/html/voices/jikkyo-' + jikkyo_id7 +'-1.wav'
		...
		if reason == 'upper_jikkyolized':
			...
			cmd = 'curl "https://api.voicetext.jp/v1/tts" -u "mpbw1hnrp32pdde7:" -d "text= \'' + vocalize_sentence + '\'" -d "speaker=hikari" -d "speed=80" -d "pitch=150" -o /var/www/html/voices/jikkyo-' + jikkyo_id7 +'-3.wav'
		elif reason == 'lower_jikkyolized':
			vocalize_sentence = 'これはとても低い値です。'
			cmd = 'curl "https://api.voicetext.jp/v1/tts" -u "mpbw1hnrp32pdde7:" -d "text= \'' + vocalize_sentence + '\'" -d "speaker=hikari" -d "speed=80" -d "pitch=150" -o /var/www/html/voices/jikkyo-' + jikkyo_id7 +'-3.wav'
		elif reason == 'max_jikkyolized':
			vocalize_sentence = 'これは今までで最高の値です。'
			cmd = 'curl "https://api.voicetext.jp/v1/tts" -u "mpbw1hnrp32pdde7:" -d "text= \'' + vocalize_sentence + '\'" -d "speaker=hikari" -d "speed=80" -d "pitch=200" -o /var/www/html/voices/jikkyo-' + jikkyo_id7 +'-3.wav'
		...
		if rel_flag != 'No_relation' and rel_flag != 'No_speciality':
			...
			if rel_flag == 'rel_max':
				...
				cmd = 'curl "https://api.voicetext.jp/v1/tts" -u "mpbw1hnrp32pdde7:" -d "text= \'' + vocalize_sentence + '\'" -d "speaker=hikari" -d "speed=110" -d "pitch=50" -o /var/www/html/voices/jikkyo-' + jikkyo_id7 +'-5.wav'
			if rel_flag == 'rel_min':
				vocalize_sentence += '最低の値を示しています。'
				cmd = 'curl "https://api.voicetext.jp/v1/tts" -u "mpbw1hnrp32pdde7:" -d "text= \'' + vocalize_sentence + '\'" -d "speaker=hikari" -d "speed=110" -d "pitch=50" -o /var/www/html/voices/jikkyo-' + jikkyo_id7 +'-5.wav'
			if rel_flag == 'rel_much':
				vocalize_sentence += 'とても高い値を示しています。'
				cmd = 'curl "https://api.voicetext.jp/v1/tts" -u "mpbw1hnrp32pdde7:" -d "text= \'' + vocalize_sentence + '\'" -d "speaker=hikari" -d "speed=110" -d "pitch=75" -o /var/www/html/voices/jikkyo-' + jikkyo_id7 +'-5.wav'
